...
Additionally, note that in Table \@ref(tab:pop-sd-table) we consider the sample standard deviation of the number of horses during the same time span as Figure \@ref(fig:province-pop).

```{r pop-prov-sd, fig.cap= "Horse populations for the province with the largest standard deviation", out.width= "70%"}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```

In Figure \@ref(fig:pop-prov-sd) we zoom in and look at the province of `r largest_sd`, which had the largest spread of values in terms of standard deviation.
